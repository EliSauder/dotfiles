...
let
  pgrepPath = if pkgs.stdenv.isDarwin then "/opt/homebrew/bin/pgrep" else "${pkgs.procps}/bin/pgrep";
in
{
  nixpkgs.overlays = [
    (final: prev: {
      tmux-sessionizer-personal = pkgs.stdenv.mkDerivation rec {
        ...
        installPhase = ''
          ...
          sed -i -e 's|basename |${pkgs.uutils-coreutils-noprefix}/bin/basename |g' $out/bin/tms
