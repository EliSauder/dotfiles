...
in
{
  options.prog = {
    ...
    floorp.setdefault = lib.mkOption {
      default = false;
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    xdg.mimeApps.defaultApplications = lib.mkIf cfg.setdefault {
      "default-web-browser" = [ "${pkgs.floorp}/share/applications/floorp.desktop" ];
      "text/html" = [ "${pkgs.floorp}/share/applications/floorp.desktop" ];
      "x-scheme-handler/http" = [ "${pkgs.floorp}/share/applications/floorp.desktop" ];
      "x-scheme-handler/https" = [ "${pkgs.floorp}/share/applications/floorp.desktop" ];
      "x-scheme-handler/about" = [ "${pkgs.floorp}/share/applications/floorp.desktop" ];
      "x-scheme-handler/unknown" = [ "${pkgs.floorp}/share/applications/floorp.desktop" ];
    };
    programs.floorp = {
      enable = true;
      package =
